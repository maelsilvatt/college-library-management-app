...
def get_registration(cnx, login=''):

    # Extracts login from connected users if not specified
    if login == '':
        login = cnx.user
...
# Gets course count
def get_course_count(cnx):

    query = f'SELECT count(*) FROM courses'

    course_count = run(query, cnx, show=True).loc[0][0]
...
def register_user(login, password, cnx, acess='user', user_data={}):
    if acess == 'user':
        query = f"""DROP USER IF EXISTS '{login}'@'%';
                    ...
                    GRANT SELECT ON `college_library_db`.`users` TO '{login}'@'%';
                    GRANT SELECT ON `college_library_db`.`books` TO '{login}'@'%';
                    GRANT SELECT, INSERT ON `college_library_db`.`book_reservations` TO '{login}'@'%';
                    GRANT SELECT ON `college_library_db`.`book_borrowings` TO '{login}'@'%';
                    GRANT SELECT ON `college_library_db`.`books_by_category` TO '{login}'@'%';
                    GRANT SELECT ON `college_library_db`.`books_by_publisher` TO '{login}'@'%';
                    GRANT SELECT ON `college_library_db`.`books_by_release` TO '{login}'@'%';
                    GRANT SELECT ON `college_library_db`.`books_by_author` TO '{login}'@'%';
                    GRANT EXECUTE ON FUNCTION `college_library_db`.`get_user_acess` TO '{login}'@'%';
                    FLUSH PRIVILEGES;
                    INSERT INTO users (login, password, acess) VALUES ('{login}', '{password}', '{user_data['registration']}');"""

        if user_data['user_type'] == 'student':
            query += f"""INSERT INTO students (registration, name, entry_date, expected_completion_date, adress, course_code)
                         VALUES ('{user_data['registration']}', '{user_data['name']}', '{user_data['entry_date']}',
                         '{user_data['expected_completion_date']}', '{user_data['adress']}', '{user_data['course_code']}');"""

        elif user_data['user_type'] == 'employee':
            query += f"""INSERT INTO employees (registration, name, adress)
                         VALUES ('{user_data['registration']}', '{user_data['name']}', '{user_data['adress']}');"""

        if user_data['user_type'] == 'professor':
            query += f"""INSERT INTO professors (registration, name, hiring_date, work_regime, adress, cell_phone, course_code)
                         VALUES ('{user_data['registration']}', '{user_data['name']}', '{user_data['hiring_date']}',
                         '{user_data['work_regime']}', '{user_data['adress']}', '{user_data['cell_phone']}', '{user_data['course_code']}');"""
        ...
        query = f"""DROP USER IF EXISTS '{login}'@'%';
                    ...
                    GRANT SELECT ON `college_library_db`.`books` TO '{login}'@'%';
                    GRANT SELECT ON `college_library_db`.`users` TO '{login}'@'%';
                    GRANT SELECT ON `college_library_db`.`students` TO '{login}'@'%';
                    GRANT SELECT ON `college_library_db`.`employees` TO '{login}'@'%';
                    GRANT SELECT ON `college_library_db`.`books_by_category` TO '{login}'@'%';
                    GRANT SELECT ON `college_library_db`.`books_by_publisher` TO '{login}'@'%';
                    GRANT SELECT ON `college_library_db`.`books_by_release` TO '{login}'@'%';
                    GRANT SELECT ON `college_library_db`.`books_by_author` TO '{login}'@'%';
                    GRANT SELECT, INSERT, DELETE, UPDATE ON `college_library_db`.`book_reservations` TO '{login}'@'%';
                    GRANT SELECT, INSERT, DELETE, UPDATE ON `college_library_db`.`book_borrowings` TO '{login}'@'%';
                    FLUSH PRIVILEGES;
                    INSERT INTO users (login, password, acess) VALUES ('{login}', '{password}', '{acess}');"""

    elif acess == 'admin':
        query = f"""DROP USER IF EXISTS '{login}'@'%';
                    CREATE USER '{login}'@'%' IDENTIFIED BY '{password}';
                    GRANT ALL ON `college_library_db`.* TO '{login}'@'%'; 
                    FLUSH PRIVILEGES;
                    INSERT INTO users (login, password, acess) VALUES ('{login}', '{password}', '{acess}');"""
...
def get_user_type(registration, cnx):
...
def borrow_book(registration, isbn_code, cnx):
    ...
    # Extracts the type from the selected user as "student"|"professor"|"employee"|"none"
    user_type = get_user_type(registration, cnx)
    ...
    if user_type == 'student':
        ...
        query = f"""INSERT INTO book_borrowings (devolution_date, registration, isbn_code) 
                    VALUES ('{devolution_date}', '{registration}', '{isbn_code}')"""
...
def reserve_book(isbn_code, cnx):
    ...
    query = f"""INSERT INTO book_reservations (registration, isbn_code) 
                VALUES ('{registration}', '{isbn_code}')"""
...
# Returns user reserved books as a list
def get_user_reserved_books(cnx):

    # Extracts the registrarion from connected user
    registration = get_registration(cnx)

    query = f"""SELECT books.title, books.isbn_code, reservation_date 
...
def get_user_borrowed_books(cnx):
    ...
    query = f"""SELECT books.title, borrow_date, devolution_date, books.isbn_code 
...
# Registers a new book to the database
def register_book(cnx, data={}):

    query = 'INSERT INTO books (isbn_code, title, release_year,' \
            'publisher, category_code, copies_available) VALUES' \
           f'("{data["isbn_code"]}", "{data["title"]}", "{data["release_year"]}", '\
           f'("{data["publisher"]}", "{data["category_code"]}", "{data["copies_available"]}")'

    run(query, cnx)


# Returns the total number of categories
def get_category_count(cnx):

    query = 'SELECT COUNT(*) FROM categories'

    run(query, cnx, show=True).loc[0][0]



# Calls a method to remove the book from the database
def remove_book(isbn_code, cnx):

    query = f'DELETE FROM books WHERE books.isbn_code="{isbn_code}"'
